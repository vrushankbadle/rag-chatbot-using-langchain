# Get Started

NOTE : The given commands are for Git bash terminal. The commands are different depending on your terminal. Please verify and use the correct commands.

### To get started with the project, follow these steps or directly run the commands in terminal :
1. Clone the repository in desired directory
    - `git clone https://github.com/vrushankbadle/rag-chatbot-using-langchain.git`
2. Create a python virtual environment 
    - Make sure you have python 3.11 installed.
    - `python3.10 -m venv .venv`
3. Activate the virtual environment
    - `source .venv/Scripts/activate`
4. Install required packages from requirements.txt
    - `pip install -r rag-chatbot-using-langchain/requirements.txt`


### To run the rag model :
1. Navigate and run rag-chatbot-using-langchain/langchain gemini.py manually or inside terminal run 
    - `python "rag-chatbot-using-langchain/main.py`

